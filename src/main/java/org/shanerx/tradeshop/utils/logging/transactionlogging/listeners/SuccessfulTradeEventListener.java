...
package org.shanerx.tradeshop.utils.logging.transactionlogging.listeners;
...
public class SuccessfulTradeEventListener implements Listener {
    ...
    public SuccessfulTradeEventListener(TradeShop tradeShop) {
    ...
    public void onSuccessfulTrade(PlayerSuccessfulTradeEvent event) {
        if (!event.isCancelled()) {
            Bukkit.getScheduler().runTaskAsynchronously(tradeShop, () -> tradeShop.getTransactionLogger().logTransaction(event));
        }
    }
}
